...
const Settings = () => {
  ...
  async function fetchBalance() {
    setAccbal("");
    let data = await axios.post("https://gatekeepers-backend.herokuapp.com/balance", {
      id: accid,
      key: privatekey,
    });
    setAccbal((data.data.data.balance._valueInTinybar / 100000000 - 0).toFixed(4));
  }

  // function to delete account from firebase and hedera network function
  async function deleteAccount() {
    await axios.post(`https://gatekeepers-backend.herokuapp.com/deleteAccount`, {
      id: accid,
      key: privatekey,
    });
    setDisableBtn(true);
      ...
      <Grid item xs={12} md={6}>
        ...
        >
          ...
          >
            ...
            <Button disabled={disableBtn} onClick={handleClickOpen} color="error" startIcon={<Delete />}>
              Delete Account
            </Button>
            <Button onClick={uploadPhoto} color="success" startIcon={<UploadIcon />}>
              Upload Photo
            </Button>

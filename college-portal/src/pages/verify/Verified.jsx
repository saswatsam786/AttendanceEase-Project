...
import { UnstyledButton, Group, Avatar, Text, Autocomplete, Menu, ScrollArea, Alert } from "@mantine/core";
...
const Verified = () => {
      ...
      try {
        const q =
          user.email === "gov@govindia.in"
            ...
            : query(collection(db, "colleges"), where("domain", "==", user.email.split("@")[1]));
  ...
  const noStudent = (scholarshipName, students) => {
    const s = students.filter((student) => student.student.scholarships.includes(scholarshipName));
    return s.length === 0;
  };

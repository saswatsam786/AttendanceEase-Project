...
import { IconFileCheck, IconEye, IconUser, IconReportMoney, IconTrophy } from "@tabler/icons";
    ...
    <div className={`${classes.studentContainer} studentContainer py-3`}>
      ...
      <Tabs variant="outline" defaultValue="profile">
        <Tabs.List grow position="center">
          <Tabs.Tab value="profile" icon={<IconUser></IconUser>} onClick={() => setTab("profile")}>
            User Profile
          </Tabs.Tab>
          <Tabs.Tab value="leave" icon={<IconEye></IconEye>} onClick={() => setTab("leave")}>
            Leave Applications
          </Tabs.Tab>
          <Tabs.Tab value="achievements" icon={<IconTrophy></IconTrophy>} onClick={() => setTab("achievements")}>
            Achievements
          </Tabs.Tab>
          <Tabs.Tab value="stats" icon={<IconFileCheck></IconFileCheck>} onClick={() => setTab("stats")}>
            Stats
          </Tabs.Tab>
          {user.email === "gov@govindia.in" && (
            <Tabs.Tab value="receipts" icon={<IconReportMoney></IconReportMoney>} onClick={() => setTab("receipts")}>
              Receipts
            </Tabs.Tab>
          )}
          {user.email === "gov@govindia.in" && (
            <Tabs.Tab value="award" icon={<IconTrophy></IconTrophy>} onClick={() => setTab("award")}>
              Award Scholarships
            </Tabs.Tab>
          )}
        </Tabs.List>
      ...
      <div className={`container`}>
        {tab === "profile" && <StudentProfileData data={data} college={college} scholarships={scholarships} />}
        ...
        {tab === "stats" && <StudentStats data={data} theme={theme}></StudentStats>}
  ...
  return (
    <div className="row my-4 g-2 gx-3">
      ...
      <div className="col-12 col-lg-4">
        <div className={`${classes.left} left ${classes.borders} ${classes.studentInfo}`}>
          <div className="top">
            <img
              alt="profile-pic"
      ...
      <div className="col-12 col-lg-8">
        <div className={`${classes.left} ${classes.borders} right py-2`}>
          <div className="row my-2 px-2">
            ...
            <div className={`${classes.rightt}  col-8 col-md-9 rightt`}>{data.student.sname}</div>
          </div>
          <hr />
          <div className="row mb-2 px-2">
            <div className="col-4 col-md-3 leftt">College ID</div>
            <div className={`${classes.rightt} col-8 col-md-9 rightt`}>{data.student.email.split("@")[0]}</div>
          </div>
          <hr />
          <div className="row mb-2 px-2">
            <div className="col-4 col-md-3 leftt">Email</div>
            <div className={`${classes.rightt} col-8 col-md-9 rightt`}>{data.student.email}</div>
          </div>
          <hr />
          <div className="row mb-2 px-2">
            <div className="col-4 col-md-3 leftt">College</div>
            <div className={`${classes.rightt} col-8 col-md-9 rightt`}>{college}</div>
          </div>
          <hr />
          <div className="row mb-2 px-2">
            <div className="col-4 col-md-3 leftt">Applications</div>
            <div className={`${classes.rightt} col-8 col-md-9 rightt`}>
              ...
              <Accordion>
                {data.student.applications.length === 0 ? (
                  ...
                  <>{data.student.applications.length}</>
                )}
              </Accordion>
            </div>
          </div>

...
const QRGenerator = () => {
  ...
  useEffect(() => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        function (error) {
          setError(error.POSITION_UNAVAILABLE);
        },
        {
  ...
  useEffect(() => {
    ...
    async function getQRCode() {
      try {
        const q = query(collection(db, "QRTokens"), where("cdomain", "==", user.email.split("@")[1]));

        const querySnapshot = await getDocs(q);
        querySnapshot.docs.forEach((doc) => {
          if (
            new Date().getTime() >= doc.data().validStartTime.toDate().getTime() &&
      ...
      {error ? (
        ...
        <>
          ...
          <div style={{ width: "80%", margin: "auto" }}>
            ...
            {!location && <TextInput label="Location" value="Loading location" disabled></TextInput>}
          </div>
